...
def set_frame(debugger, command, result, internal_dict):
  ...
  if len(args) != 5:
    result.Print('ERROR: Please enter the command as "setframe <instance> <x> <y> <width> <height>".')
    return
  ...
  if temp_result.GetOutput().strip() == 'NO':
    result.Print('ERROR: This command only works for objects that respond to the `setFrame:` selector.')
    return
